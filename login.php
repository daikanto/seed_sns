<?php 
  ...
  if (!empty($_POST)) {
      ...
      if ($email!=''&&$password!='') {
          //ログイン処理
          $sql=sprintf('SELECT * FROM `members` WHERE `email`="%s" AND `password`="%s"',mysqli_real_escape_string($db,$email),mysqli_real_escape_string($db,sha1($password)));
        ...
        <form method="post" action="" class="form-horizontal" role="form">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-8">
              ...
              <?php if (isset($error['login'])&&$error['error']=='blank'):?> 
                <p class="error">メールアドレスとパスワードを入力してください</p>
              <?php endif; ?>
              <?php if (isset($error['login'])&&$error['error']=='failed'):?><p class="error">ログインが失敗しました。再度ログインしてください</p>
              <?php endif; ?>
            </div>
          </div>
          <!-- パスワード -->
